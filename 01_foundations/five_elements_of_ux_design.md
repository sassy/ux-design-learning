...
ここで大切なのは「誰のどんな問題を解決するのか」を深掘りすること。
深掘りに大事なのは「解決する」という動詞は使わない。これは作り手が主語だから。
戦略はユーザーが何に困っているかというユーザーの現実の状態を扱うレイヤーなので、ユーザーの困りごとにフォーカスする。

「できない」ではなく「なぜできないか」まで書くことが大事
